...
static void test_parse_string(void) {
    ...
    TEST_STRING("Hello\nWorld", "\"Hello\\nWorld\"");
    TEST_STRING("\" \\ / \b \f \n \r \t", "\"\\\" \\\\ \\/ \\b \\f \\n \\r \\t\"");
    /*In Unicode encoding, \u0000 represents a null character, which
    is a control character, also called a non-visible character. */
...
static void test_parse_object(void) {
    ...
    for (i = 0; i < lept_get_object_value(&v, 5)->a.size; i++) {
...
static void test_access_array(void) {
    ...
    for (i = 0; i < 5; i++) {
        lept_init(&v.a.e[i]);
        lept_set_number(&v.a.e[i], (double)i);
        v.a.size++;
    }
    EXPECT_EQ_SIZE_T(5, lept_get_array_size(&v));
    EXPECT_EQ_SIZE_T(5, lept_get_array_capacity(&v));
    lept_free(&v);
}
...
static void test_access_object(void) {
    ...
    EXPECT_EQ_SIZE_T(5, lept_get_object_capacity(&o));
    for (i = 0; i < 10; i++) {
        char key[2] = "a";
        key[0] += i;
        lept_init(&v);
        lept_set_number(&v, i);
        lept_free(lept_set_object_value(&o, key, 1));
        memcpy(lept_set_object_value(&o, key, 1), &v,sizeof(lept_value));
        o.o.size++;
        lept_free(&v);
    }
